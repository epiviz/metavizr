...
matrix sizes with YYY elements perform well for interactive visualization with an R session using WebSockets.  For larger abundance matrices, we recommend using the graph database backend available at [NEED TO ADD LINK].  Once having subset the data, we normalize count data using the `metagenomeSeq::cumNorm` function, choose genera that have more than 5 OTUs in the dataset and select the 10 genera with largest standard deviation of log-transformed normalized counts across samples. Set aggregation level to "class", will find features DA at this level and then change an push those changes to an MRexperiment that is aggreagted to "species" level, and then explore the hiearchy.
...
aggregated_species <-  cumNorm(aggregateByTaxonomy(bangladesh_species, lvl=aggregation_level), p = 0.75)
varLabels(featureData(aggregated_species)) <- aggregation_level
temp <- unique(fData(bangladesh_species)[,aggregated_feature_order])
temp[is.na(temp)] <- "no_match"
temp <- temp[order(temp[,aggregation_level]),]
row.names(temp) <- temp[,aggregation_level]
featureNames(featureData(aggregated_species)) <- as.character(temp[,aggregation_level])
fData(aggregated_species) <- temp[order(row.names(MRcounts(aggregated_species))),]

aggregation_level <- "class"
aggregated_bangladesh <- aggregateByTaxonomy(bangladesh, lvl=aggregation_level)
varLabels(featureData(aggregated_bangladesh)) <- aggregation_level
temp <- unique(fData(bangladesh)[,feature_order[1:which(feature_order == aggregation_level)]])
temp[is.na(temp)] <- "no_match"
temp <- temp[order(temp[,aggregation_level]),]
row.names(temp) <- temp[,aggregation_level]
featureNames(featureData(aggregated_bangladesh)) <- as.character(temp[,aggregation_level])
fData(aggregated_bangladesh) <- temp[order(row.names(MRcounts(aggregated_bangladesh))),]
...
fSelection <- rep(2, length(feature_names))
names(fSelection) <- feature_names
...
feature_order2 <- feature_order[1:7]
...
aggregated_etec16s <- aggregateByTaxonomy(etec16s, lvl = aggregation_level)
varLabels(featureData(aggregated_etec16s)) <- aggregation_level
temp <- unique(fData(etec16s)[,feature_order2])
temp[is.na(temp)] <- "no_match"
temp <- temp[order(temp[,aggregation_level]),]
temp <- temp[!duplicated(temp[,aggregation_level]),]
row.names(temp) <- temp[,aggregation_level]
featureNames(featureData(aggregated_etec16s)) <- as.character(temp[,aggregation_level])
fData(aggregated_etec16s) <- temp[order(row.names(MRcounts(aggregated_etec16s))),]
splinesMRexp <- timeSeriesToMRexperiment(timeSeriesFits, feature_data = featureData(aggregated_etec16s), sampleNames = timeSeriesFits[[2]]$fit$timePoints)
```
...
splineObj <- app$data_mgr$add_measurements(splinesMRexp[splines_to_plot_indices,], datasource_name = "splines", control = metavizControl(norm=FALSE, aggregateAtDepth = 6), feature_order = feature_order2)
