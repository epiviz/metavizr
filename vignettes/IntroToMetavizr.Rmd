...
*NOTE:* The following commands are not visible when building the vignette.  These commands are for registering each chart type so that the vignette will work when starting a new Metaviz session using localhost.  These commands are not needed if using `startMetaviz` with default which starts a Metaviz session using http://metaviz.cbcb.umd.edu.
...
A metavizControl allows users to specify settings for charts and data analysis such as normalization and log transform.  We create a metavizControl with the featureSelection as the nodes that be found to be differentially abundant and those will be visible when creating Metaviz plots.
