...
app <- startMetaviz(host="http://localhost:7200")
```
...
species_level_agg <-  cumNorm(aggregateByTaxonomy(bangladesh, feature_order = feature_order, lvl="species"), p = 0.75)
aggregated_feature_order <- c("superkingdom", "phylum", "class", "order", "family", "genus", "species")

aggregation_level <- "class"
aggregated_bangladesh <- aggregateByTaxonomy(bangladesh, lvl=aggregation_level, feature_order = feature_order)
...
logFC_bangladesh <- MRcoefs(results_bangladesh, number = dim(normed_bangladesh)[1])


fSelection <- list()
fSelection$featureNames <- rownames(logFC_bangladesh[which(logFC_bangladesh[which(abs(logFC_bangladesh$logFC) > 2),]$adjPvalues < .05),])
fSelection$featureOrder <- feature_order
fSelection$featureLevel <- aggregation_level
fSelection$selectionType <- 2
...
icicle_plot <- app$plot(species_level_agg, datasource_name="mmssdd", feature_order = aggregated_feature_order, control=control)
```
...
```{r, eval=TRUE}
fSelection2 <- list()
fSelection2$featureNames <- rownames(logFC_bangladesh[which(logFC_bangladesh[which(abs(logFC_bangladesh$logFC) > 1),]$adjPvalues < .05),])
fSelection2$featureOrder <- feature_order
fSelection2$featureLevel <- aggregation_level
fSelection2$selectionType <- 2
app$get_ms_object(icicle_plot)$featureSelection(featureNames=fSelection2$featureNames, featureOrder=fSelection2$featureOrder, featureLevel=fSelection2$featureLevel, selectionType=fSelection2$selectionType)
```
