...
In this vignette, we will explore the humanized gnotobiotic mouse gut data from Turnbaugh et al. that is released as an `MRexperiment` object in the `metagenomeSeq` package [1].  In the experiment, twelve gnotobiotic adult male mice were fed a low-fat, plant polysaccharide–rich diet. Each mouse then received a fecal transplant of healthy adult human fecal material.  All mice continued on the low-fat, plant polysaccharide–rich diet for 4 weeks.  A subset of six mice were  then switched to a high-fat and high-sugar diet for 8 weeks. The V2 region of the bacterial 16s rRNA gene was PCR ampilified in fecal samples taken weekly for each mouse.
...
To start our analysis we normalize count data using the `metagenomeSeq::cumNorm` function,
choose genera that have more than 5 OTUs in the dataset and select the 10 genera with largest 
standard deviation of log-transformed normalized counts across samples.

```{r, eval=TRUE}
# normalize data using CSS at 3rd quartile
md <- cumNorm(mouseData, p=.75)

# keep genera that have more than 5 OTUs in dataset
taxonomy_table <- pData(featureData(md))
otus_per_genus <- table(taxonomy_table$genus)
genera_to_keep <- names(otus_per_genus)[otus_per_genus > 5]

# aggregate counts to genus level and log-transform
genus_counts <- aggTax(md, lvl="genus", out="matrix")[genera_to_keep,]
log_genus_counts <- log2(genus_counts + 1)

# select the 10 genera with larges variance
genus_sds <- matrixStats::rowSds(log_genus_counts)
o <- order(genus_sds)
genera_to_keep <- rownames(genus_counts)[o[1:10]]

# filter the dataset to the OTUs in those genera
otus_to_keep <- taxonomy_table$genus %in% genera_to_keep
filtered_md <- md[otus_to_keep,]

taxonomy <- pData(featureData(filtered_md))
for (i in seq(len=ncol(taxonomy))) {
  taxonomy[,i] <- factor(taxonomy[,i])
}
pData(featureData(filtered_md)) <- taxonomy
```
...
```{r,eval=TRUE}
icicle_plot <- app$plot(filtered_md, datasource_name="mouseData")
```
...
Next, we will add an interactive heatmap to visualize count data from the filtered 
data object we have created. Let's choose all mouse samples whose diet is "Western"

```{r,eval=TRUE}
western_ids <- which(phenoData(filtered_md)$diet == "Western")
```

Now we have the OTU Matrix, lets add this data to metavizr as a new data source.

```{r, eval=TRUE}
# first access the measurements object we created when plotting the icicle
md_obj <- app$get_ms_object(icicle_plot)
measurements <- md_obj$get_measurements()[western_ids]
heatmap <- app$chart_mgr$visualize("HeatmapPlot", md_obj, measurements=measurements)
```
...
```{r, eval=TRUE}
stackedPlot <- app$chart_mgr$visualize("StackedLinePlot", md_obj, measurements=measurements)
```
...
Another feature of metavizr is to visualize data using a line plot.  To illustrate, we describe a use case with data from Pop et al. [2].  In the study, 12 subjects were exposed to enterogenic E. coli and then treated with antibiotics.  Samples were collected at 2 days before treatment and then on each day afterwards for 10 days.  Paulson et al. describe analyzing this time series data with a smoothing-spline [3].  We detail the steps to perform this analysis and create a line plot using Metaviz. 
...
```{r, eval=TRUE}
splinesMRexp <- ts2MRexperiment(timeSeriesFits, featuresOfInterest = featuresOfInterest)
```

Finally, we add the MRexperiment as a measurement for Metaviz to plot.

```{r, eval=TRUE}
splineObj <- app$data_mgr$add_measurements(splinesMRexp, datasource_name = "splines", control = metavizControl(norm=FALSE))
...
[1] Turnbaugh, P.J., Ridaura, V.K., Faith, J.J., Rey, F.E., Knight, R. and Gordon, J.I., 2009. The effect of diet on the human gut microbiome: a metagenomic analysis in humanized gnotobiotic mice. Science translational medicine, 1(6), pp.6ra14-6ra14.
