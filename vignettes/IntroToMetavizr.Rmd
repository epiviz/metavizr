...
feature_names <- rownames(logFC_bangladesh[which(logFC_bangladesh[which(abs(logFC_bangladesh$logFC) > 2),]$adjPvalues < .05),]) 
...
fSelection2 <- generateSelection(feature_names =  feature_names2, aggregation_level = aggregation_level, selection_type =2)
...
```{r, eval=TRUE}
feature_data <- featureData(aggregateByTaxonomy(etec16s, lvl="Species", feature_order = feature_order[1:7])[names(timeSeriesFits)[-which(names(timeSeriesFits) == 'call')],])

splinesMRexp <- ts2MRexperiment(timeSeriesFits, feature_data = feature_data, sampleNames = timeSeriesFits[[1]]$fit$timePoints)
```

Finally, we add the MRexperiment as a measurement for Metaviz to plot.

```{r, eval=TRUE}
splines_to_plot <- sapply(1:nrow(MRcounts(splinesMRexp)), function(i) {max(abs(MRcounts(splinesMRexp[i,]))) >= 2.5})
splines_to_plot_indices <- which(splines_to_plot == TRUE)

ic_plot <- app$plot(splinesMRexp[splines_to_plot_indices,], datasource_name = "etec16_base", control=metavizControl(norm = FALSE, aggregateAtDepth = 6), feature_order = feature_order[1:7])
splineObj <- app$data_mgr$add_measurements(splinesMRexp[splines_to_plot_indices,], datasource_name = "splines", control = metavizControl(norm=FALSE, aggregateAtDepth = 6))
