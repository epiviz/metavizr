...
```{r, eval=TRUE}
timeSeriesFits = lapply(featuresOfInterest,function(i){
...
```{r, eval=TRUE}
sampleNames <- paste("Timepoint", 1:11, sep="_")
sampleDescription <- "timepoints"
featureNames <- bacteria
featureHierarchyRoot <- "bacteria"
featureDescription <- "taxonomy"
splinesMRexp <- toMRexperiment(timeSeriesFits, sampleNames = sampleNames, sampleDescription = sampleDescription, featureNames = featureNames, featureDescription = featureDescription)
```
