...
In this vignette, we will explore data from 12 human subjects that were exposed to enterogenic E. coli and then treated with antibiotics.  Samples were collected at 1 day before treatment and then on each day afterwards for 9 days.  The experiment is described in Pop et al. [1] and the data is available as an `MRexperiment` in the `etec16s` Bioconductor package.  
...
```{r, eval=TRUE}
data_obj <- etec16s
#change the OTU.ID from 'integer' to 'character'
featureData(data_obj)$OTU.ID <- sapply(featureData(data_obj)$OTU.ID, as.character)

#provide a hierarchy root node for dataset
featureData(data_obj)$Kingdom <- "Bacteria"

# normalize data using CSS at 3rd quartile
md <- cumNorm(data_obj, p=.75)

# keep species that have more than 5 OTUs in dataset
taxonomy_table <- pData(featureData(md))
otus_per_species <- table(taxonomy_table$Species)
species_to_keep <- names(otus_per_species)[otus_per_species > 5]

# aggregate counts to species level and log-transform
species_counts <- aggTax(md, lvl="Species", out="matrix")
log_species_counts <- log2(species_counts + 1)

# select the 50 species with larges variance
species_sds <- matrixStats::rowSds(log_species_counts)
o <- order(species_sds)
species_of_interest <- rownames(species_counts)[o[1:50]]

# filter the dataset to the OTUs in those species
otus_to_keep <- taxonomy_table$Species %in% species_of_interest
...
icicle_plot <- app$plot(filtered_md, feature_order = feature_order, datasource_name="etec16sData")
```
...
measurements <- md_obj$get_measurements()
...
![etec 16s metaviz workspace](images/Metaviz.png)

# Longitudinal Data Analysis

Another feature of metavizr is to visualize data using a line plot.  We detail the steps to perform this analysis and create a line plot using Metaviz such as those used in Paulson et al. when analyzing this time series data with a smoothing-spline [2].
