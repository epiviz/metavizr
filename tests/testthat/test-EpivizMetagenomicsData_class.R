...
test_that("getValuesOrder", {
  ...
  res <- mObj$getValues(measurement = colnames(pData(msd16s)), start=1, end=26044, selectedLevels = 4)
  ...
  intersect_result <- intersect(round(res$cols[[sampleId]], digits=3), round(expected,digits=3))
  expect_equal(length(intersect_result), length(expected))
})
...
test_that("getValuesFamily", {
  ...
  sampleId<- "600796"
  mObj <- metavizr:::EpivizMetagenomicsData(cumNorm(msd16s, p=.75), feature_order = feature_order)
  res <- mObj$getValues(measurement = colnames(pData(msd16s)), start=1, end=26044, selectedLevels = 5)
...
test_that("getValuesGenus", {
  ...
  sampleId<- "600796"
  mObj <- metavizr:::EpivizMetagenomicsData(cumNorm(msd16s, p=.75), feature_order = feature_order)
  res <- mObj$getValues(measurement = colnames(pData(msd16s)), start=1, end=26044, selectedLevels = 6)
...
test_that("getValuesSpecies", {
  ...
  sampleId<- "600796"
  mObj <- metavizr:::EpivizMetagenomicsData(cumNorm(msd16s, p=.75), feature_order = feature_order)
  res <- mObj$getValues(measurement = colnames(pData(msd16s)), start=1, end=26044, selectedLevels = 7)
